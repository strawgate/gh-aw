...
// Mock generate_footer
vi.mock("./generate_footer.cjs", () => ({
  generateFooter: vi.fn().mockReturnValue("\n\n---\n_Generated by workflow_"),
}));
...
describe("pr_review_buffer", () => {
  let buffer;

  beforeEach(() => {
    vi.clearAllMocks();

    // Re-require to get fresh module reference (state is shared via reset())
    buffer = require("./pr_review_buffer.cjs");
    buffer.reset();
  });
  ...
  describe("setReviewContext", () => {
    it("should set context on first call", () => {
      const ctx = {
    ...
    it("should not override context on subsequent calls", () => {
      const ctx1 = {
        repo: "owner/repo1",
      ...
      buffer.setReviewContext(ctx1);
      buffer.setReviewContext(ctx2);

      // Only the first context should have been logged
  ...
  describe("hasBufferedComments / getBufferedCount", () => {
  ...
  describe("submitReview", () => {
    it("should skip when no comments are buffered", async () => {
    ...
    it("should submit review with metadata when set", async () => {
      ...
      mockGithub.rest.pulls.createReview.mockResolvedValue({
        data: { id: 200, html_url: "https://github.com/owner/repo/pull/42#pullrequestreview-200" },
      });
    ...
    it("should include multi-line comment fields with side fallback for start_side", async () => {
      ...
      mockGithub.rest.pulls.createReview.mockResolvedValue({
        data: { id: 300, html_url: "https://github.com/owner/repo/pull/42#pullrequestreview-300" },
      });
    ...
    it("should append footer when footer context is set", async () => {
      ...
      buffer.setFooterContext({
        ...
        workflowSource: "test.md",
        workflowSourceURL: "https://github.com/owner/repo/blob/main/test.md",
      });

      mockGithub.rest.pulls.createReview.mockResolvedValue({
        data: { id: 400, html_url: "https://github.com/owner/repo/pull/42#pullrequestreview-400" },
      });
      ...
      expect(callArgs.body).toContain("Generated by workflow");
    });
    ...
    it("should submit multiple comments in a single review", async () => {
      ...
      mockGithub.rest.pulls.createReview.mockResolvedValue({
        data: { id: 500, html_url: "https://github.com/owner/repo/pull/42#pullrequestreview-500" },
      });
